classdef RecordingGroup < handle
    ...
    methods (Hidden)
        ...
        function harmonizeMetadata(rg, recording_array)
            ...
            for r = 1:length(recording_array)
                missing_field = unique_fields(~contains(unique_fields,md_fields{end}));
    ...
    methods (Static)
       ...
       function plot_cluster_outlines(reduction, cluster_idx, ax, plot_centroid, nodeSz, mapSz, sigma, cmap)
           arguments
               reduction {isnumeric}
           ...
           for i = 1:max(cluster_idx)
               ...
               coori = reduction(idx,:);
               if plot_centroid
                   s = scatter(coori(:,1),coori(:,2),nodeSz,repmat(cmap(i,:),sum(idx),1),'o','filled', 'MarkerEdgeColor','none','MarkerFaceAlpha',0.5);
...
%                 end
        ...
        function reduction = reduceDimensionality(rg, level, method, n_dims, unit_features, network_features, feature_names, useClustered,...
...
%                     n_neighbors = 15;
                    ...
                    [reduction, umap, clusterIdentifiers, extras] = run_umap(norm_data,'n_components',2,'n_neighbors',100,'min_dist',0.1,'cluster_detail','adaptive','spread',1,'sgd_tasks',20,...
...
%                         Y_test = true_value(test_idx);
        ...
        function cluster_idx = clusterByFeatures(rg, input_type, level, method, N_clust, grouping_var)
           arguments
                ...
                N_clust {isnumeric} = nan %Number of imposed clusters // 0 selects N_clust by finding the optimal silhouette score
...
%                fprintf('Modularity: %.2f\n',Q)
           ...
           figure('Color','w');
           RecordingGroup.plot_cluster_outlines(feat_mat, cluster_idx)
        ...
        function cluster_purity = calculateClusterPurity(rg, input_type, method, cluster_vars, pooling_vals)
           ...
           n_points = length(predicted_labels);
           cluster_sizes = zeros(n_clusters, 1);
           for i = 1:n_clusters
               cluster_sizes(i) = sum(predicted_labels == unique_clusters(i));
           end
           ...
           for i = 1:n_clusters
               ...
               cluster_indices = find(predicted_labels == unique_clusters(i));
               cluster_labels = ground_truth_labels(cluster_indices);
               label_counts = histcounts(cluster_labels, 1:n_clusters+1);
               [~, max_idx] = max(label_counts);
               majority_class = cluster_labels(max_idx);
               
               % Compute the purity of the cluster
               majority_count = sum(cluster_labels == majority_class);
               cluster_purity = cluster_purity + majority_count;
           end
           cluster_purity = cluster_purity / n_points;
        end
...
%                 clust_counts = histcounts(Y_train);
%                 max_clust_sz = max(clust_counts);
%                 resampled_mat = [];
%                 Y_train_resampled = [];
%                 
%                 for c = 1:length(unique(Y_train))
%                     train_id = find(train_idx);
%                     clust_idx = train_id(Y_train == c);
%                     full_resample = floor(max_clust_sz/clust_counts(c));
%                     resampled_idx = repmat(clust_idx,[full_resample 1]);
%                     additional_resample = max_clust_sz - length(resampled_idx);
%                     additional_idx = datasample(clust_idx, additional_resample, 'Replace',false);
%                     full_idx = [resampled_idx; additional_idx];
%                     resampled_mat(end+1:end+length(full_idx),:) = input_table(full_idx,:).Variables;
%                     Y_train_resampled(1,end+1:end+length(full_idx)) = ones(size(full_idx)) * c;
%                 end
%                 
%                 test_mat = input_table(test_idx,:).Variables;
%                 train_idx = logical([ones(size(Y_train_resampled)), zeros(1,sum(test_idx))]);
%                 test_idx = ~train_idx;
%                 input_table = array2table([resampled_mat; test_mat],'VariableNames',input_table.Properties.VariableNames);
...
% predImp = [];
        ...
        function [cluster_idx, group_labels_comb] = plot_true_clusters(rg, level, method, grouping_var, pooling_vals, plot_centroid, nodeSz, mapSz, sigma, cmap)
            arguments
                rg RecordingGroup
                level string = "Unit" %Unit, Recording, Culture
            ...
            if isempty(cmap)
                cmap = othercolor('Mdarkrainbow',max(cluster_idx));
            end
            ax = gca;
            RecordingGroup.plot_cluster_outlines(reduction, cluster_idx, ax, plot_centroid, nodeSz, mapSz, sigma, cmap);
        ...
        function [colors, avg_wf] = plot_cluster_waveforms(rg,method)
            arguments
                rg RecordingGroup
                method string = "kmeans"
            end
...
%             aligned_wf = rg.alignWaveforms([rg.Units]);
            ...
            figure("Color","w");
            ax = nexttile;
            avg_time_vector = linspace(min(time_vector),max(time_vector),200);
...
%             p = plot(time_vector,horzcat(avg_wf{:}),"LineWidth",2);
        ...
        function [feature_table, mean_mat, sd_mat, group_labels_comb] = plot_feature_trajectories(rg, level, grouping_var, grouping_values, network_features, unit_features, normalization,...
            ...
            jitter = linspace(-min_diff/10,min_diff/10,N_groups);
        ...
        function [value_array, group_labels_comb] = plot_cluster_densities(rg, level, method, grouping_var, pooling_vals, n_bins)
            arguments
                rg RecordingGroup
                level string = "Unit" %Unit, Recording, Culture
                method string = "UMAP" %dim reduction method: "UMAP","PCA"
