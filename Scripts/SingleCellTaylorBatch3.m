...
rg_params.Selection.Inclusion = {{'Source','Taylor'},{'Mutation',"WT"},{'DIV',56},{'Timepoint',0}}; %Cell array of cell arrays with fieldname + value // empty defaults to including all recordings
rg_params.Selection.Exclusion = {{'Treatment',"LNA"}}; %Cell array of cell arrays with fieldname + value
ctrl_untreated = RecordingGroup(taylor_array, rg_params);

%% FCDI recording group
...
fcdi_rg = RecordingGroup(fcdi_array, rg_params);

%% Taylor recording group
...
taylor_rg = RecordingGroup(taylor_array, rg_params);
...
rg_params.Selection.Exclusion = {}; %Cell array of cell arrays with fieldname + value
comb_rg = RecordingGroup([taylor_rg.Recordings, fcdi_rg.Recordings], rg_params);
...
pooling_vals = {{"A53T"    "Control"    "SNCA"    "WT"},{["Untreated","ntLNA"],["LNA"]}};
...
act_data = [act_data([1,2],:); histcounts(vertcat(ctrl_act.cluster_idx)); act_data([3:end],:)]; %ctrl_act.prediction.Y_pred
...
%% Plot waveforms
[ctrl_wf.colors, ctrl_wf.mean_wfs] = ctrl_untreated.plot_cluster_waveforms(clust_method);
