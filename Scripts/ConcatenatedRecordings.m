...
path_logic = {'230203','How_*','M*','w*','sorted'}; %Each cell corresponds to one subdirectory
...
split_times = [0 10; 10 35; 35 60]; %Cutouts of the recording 
...
parfor iPath = 1:length(sorting_path_list)
    ...
    if exist(spkt_file,'file')
        ...
        if round(max(spk_t)/600000) == 45
            fprintf('Skipped recording %s \n', sorting_path);
            continue
        end
        output_folder = "full_min_" + split_times(:,1); %Name folders according to the minute the cutout starts
        ...
        for iSplit = 1:numel(output_path_list)
            ...
            metadata.Parameters.Save.Flag = true;
