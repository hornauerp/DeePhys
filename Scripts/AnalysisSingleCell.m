...
wt_untreated.reduceDimensionality(dr_level, dr_method, n_dims, act_unit_features, network_features, feature_names, useClustered,...
    normalization_var, grouping_var, grouping_values, normalization, tolerance);

%% Generate clusters
wt_untreated.clusterByFeatures(dr_method,dr_level,clust_method, resolution_parameter);

%% Remove units with 0 activity
wt_untreated.assignUnitClusterIdx("louvain",false,false);
wt_untreated.removeUnitsByCluster("louvain",1);
...
save(fullfile(save_path,'activity_prediction.mat'),'act')
...
save(fullfile(save_path,'waveform_prediction.mat'),'wf')
...
save(fullfile(save_path,'combined_prediction.mat'),'comb')
